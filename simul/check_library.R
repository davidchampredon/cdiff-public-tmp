  ...
  prm.hosp <- full.param.HCS(file_infrastruct = 'rooms_hosp.csv', 
                             file_epi         = 'prm_hosp.csv') 
  
  prm.ltcf <- full.param.HCS(file_infrastruct = 'rooms_ltcf.csv', 
                             file_epi         = 'prm_ltcf.csv') 
  
  hcs.prm <- list(prm.hosp, prm.ltcf)
  
  # Movements between HCS:
  mvt_row_1 <- c(0, 0.5)
  mvt_row_2 <- c(0.1, 0)
  mvthcs.prm <- list(mvt_row_1, mvt_row_2)
  
  sim.prm <- list(horizon = 100, 
                 timestep = 0.5,
  ...
  res <- abmcdiff_test(hcs.prm, sim.prm, disease.prm, mvthcs.prm)
  ...
  # Check if all individuals are not
  # simulataneously in 2 different HCS:
  df.mvt <- df %>%
    group_by(time, type, uid) %>%
    summarize(n = length(unique(hcs_name))) 
  stopifnot(all(df.mvt$n==1))
